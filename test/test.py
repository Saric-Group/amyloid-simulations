...
lmp.cmd.region("box", "block", xmin, xmax, ymin, ymax, zmin, zmax)
simulation.setup("box")

simulation.create_rods(state_ID = 1, exact = [fibril_rods])

lmp.cmd.fix("thermostat", "all", "langevin", T, T, damp, seed)
...
lmp.cmd.dump("dump_cmd", "all", "custom", output_freq, '"' + dump_path + '"', dump_elems)
...
lmp.cmd.thermo(mc_every)

lmp.cmd.neigh_modify("every 1 delay 1")
lmp.cmd.timestep(0.01)
lmp.cmd.run(1000)
