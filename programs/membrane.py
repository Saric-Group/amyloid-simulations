...
class Membrane(object):
    ...
    def create_membrane(self, lmp, seed, append = True):
        '''
        ...
        all_atom_mol_ids = lmp.gather_atoms_concat("molecule", 0, 1)
...
#                 self.type_offset, self.bond_offset, self.angle_offset))
        lmp.cmd.read_data('"' + temp_dat_file + '"', ' '.join(read_data_args))
        
        lmp.cmd.group(Membrane.top_layer_group, "molecule",
                     "{:d}:{:d}:2".format(mol_offset+1, mol_offset+membrane.mol_id))
        lmp.cmd.group(Membrane.bottom_layer_group, "molecule",
                     "{:d}:{:d}:2".format(mol_offset+2, mol_offset+membrane.mol_id))
        lmp.cmd.group(Membrane.main_group, "type", *membrane.bead_types)
...
lmp.cmd.units('lj')
lmp.cmd.dimension(3)
lmp.cmd.boundary('p p f')
lmp.cmd.region('box', 'block',
...
lmp.cmd.angle_style('harmonic')

zwalls_fix = 'zwalls'
lmp.cmd.fix(zwalls_fix, 'all', 'wall/lj126',
...
for bead_type in membrane.bead_types:
    lmp.cmd.mass(bead_type, membrane.bead_mass)
lmp.cmd.angle_coeff(membrane.angle_type, 5.0 * membrane.eps, 180)
lmp.cmd.bond_coeff(membrane.bond_type, 'fene', 30.0 * membrane.eps / membrane.sigma**2,
                  1.5 * membrane.sigma, membrane.eps, membrane.sigma)

lj_factor = pow(2, 1./6)
# head-head & head-tail inter-lipid interaction
for bead_type in membrane.bead_types:
    lmp.cmd.pair_coeff(membrane.head_type, bead_type, membrane.eps,
...
for i in range(len(membrane.tail_types)):
    for j in range(i, len(membrane.tail_types)):
        lmp.cmd.pair_coeff(membrane.tail_types[i], membrane.tail_types[j], membrane.eps,
...
lmp.cmd.pair_coeff(mem_type_range, rod_type_range, 0.0, model.global_cutoff - 0.01, model.global_cutoff)
...
for vx_type in vx_types:
    ...
    for mem_bead_type in membrane.bead_types:
        lmp.cmd.pair_coeff(mem_bead_type, vx_type + simulation.type_offset,
...
for tip_type in tip_types:
    for mem_bead_type, k in zip(membrane.bead_types, int_factors):
        lmp.cmd.pair_coeff(mem_bead_type, tip_type,
...
lmp.cmd.fix(thermo_fix, 'all', 'langevin', run_args.temp, run_args.temp, run_args.damp, seed)#, "zero yes")

simulation.set_rod_dynamics('nve', opt = ['mol', model.rod_states[0]])

lmp.cmd.region('gcmc_init', 'block',
...
lmp.cmd.variable('mem_and_tips', 'atom', '"' + 
                ' || '.join(['(type == {:d})'.format(t)
                             for t in tip_types + membrane.bead_types])
                + '"')
lmp.cmd.group(adsorbed_group, 'dynamic', 'all', 'var', 'mem_and_tips',
             'every', out_freq)
adsorbed_compute = 'mem_cluster'
lmp.cmd.compute(adsorbed_compute, adsorbed_group, 'aggregate/atom', tip_lipid_cutoff)
top_msd_compute = 'mem_top_msd'
lmp.cmd.compute(top_msd_compute, membrane.top_layer_group, 'msd')
bottom_msd_compute = 'mem_bottom_msd'
lmp.cmd.compute(bottom_msd_compute, membrane.bottom_layer_group, 'msd')
full_msd_compute = 'mem_full_msd'
lmp.cmd.compute(full_msd_compute, membrane.main_group, 'msd')

# FIXES & DYNAMICS
mem_dyn_fix = 'mem_dynamics'
press_eq_t = 1000 * run_args.dt
lmp.cmd.fix(mem_dyn_fix, membrane.main_group, 'nph',
           'x 0.0 0.0', press_eq_t, 'y 0.0 0.0', press_eq_t, 'couple xy',
           'dilate', 'all')
lmp.cmd.compute_modify(mem_dyn_fix + '_temp', 'dynamic/dof yes')
lmp.cmd.neigh_modify('exclude', 'molecule/intra', membrane.main_group)
...
lmp.cmd.special_bonds('fene') # not really necessary because of "neigh_modify exclude"

# MEMBRANE EQUILIBRATION
lmp.cmd.run(2000) #2 x press_eq_t, for good measure
lmp.cmd.reset_timestep(0)

# ===== FINAL ===========================================================================
lmp.cmd.variable('temp_x', 'equal', '"xhi - {:f}"'.format(model.rod_length / 2))
lmp.cmd.variable('temp_y', 'equal', '"yhi - {:f}"'.format(model.rod_length / 2))
lmp.cmd.region('gcmc_box', 'block',
              '-${temp_x}', '${temp_x}',
              '-${temp_y}', '${temp_y}',
...
lmp.cmd.variable("area", "equal", "lx*ly")
lmp.cmd.thermo_style("custom", "step atoms", "pe temp", "press lx ly v_area")
lmp.cmd.thermo(out_freq)
# MSD data (only in x & y directions)
lmp.cmd.variable("top_msd", "equal", '"c_{0}[1] + c_{0}[2]"'.format(top_msd_compute))
lmp.cmd.fix("mem_top_msd", "all", "ave/time", 1, 1, out_freq, "v_top_msd",
           "file", os.path.join(output_folder, sim_ID + '_mem_top.msd'))
lmp.cmd.variable("bottom_msd", "equal", '"c_{0}[1] + c_{0}[2]"'.format(bottom_msd_compute))
lmp.cmd.fix("mem_bottom_msd", "all", "ave/time", 1, 1, out_freq, "v_bottom_msd",
           "file", os.path.join(output_folder, sim_ID + '_mem_bottom.msd'))
lmp.cmd.variable("full_msd", "equal", '"c_{0}[1] + c_{0}[2]"'.format(full_msd_compute))
lmp.cmd.fix("mem_full_msd", "all", "ave/time", 1, 1, out_freq, "v_full_msd",
