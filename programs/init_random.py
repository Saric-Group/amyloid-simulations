...
The initial locations and orientations of the rods are randomly selected and minimisation
is used to remove overlapping before the simulation is started.
...
parser.add_argument('box_size', type=float, help='size of the simulation box (same in all dims)')
parser.add_argument('num_rods', type=int, help='total number of random rods to generate')
...
dump_path = str(args.box_size)+'-'+str(args.num_rods)+'_'+str(args.seed)+'.dump'
...
py_lmp.region("box", "block", -args.box_size / 2, args.box_size / 2,
                              -args.box_size / 2, args.box_size / 2,
                              -args.box_size / 2, args.box_size / 2)

simulation.setup("box")
simulation.create_rods(random = (args.num_rods, args.seed, "box"))

py_lmp.fix("thermostat", "all", "langevin", args.temp, args.temp, args.damp, args.seed)
